...
# Then in the frontend, set USE_FASTAPI = true and API base URL to
# http://localhost:8000 (see api-client.ts).
...
# Allow the Lovable-hosted frontend (and local dev) to call this API.
# Tighten this to your real frontend domain before shipping to production.
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
def search(query: SearchQuery) -> SearchResult:
    ...
    for t in train_rows:
        ...
        # Rough duration-in-minutes parse for the ranking penalty; falls back
        # to a neutral value if the stored format can't be parsed.
        try:
