...
def _route_search_core(source_q, dest_q, travel_class="SL", date_str=None) -> dict:
    ...
    if candidates:
        ...
        for c in candidates:
            ...
            scored.append((total, {
                ...
                "score": total,
                "why": f"{reason} ~{h}h {m}m, boards {board.get('code')}",
            }))
    ...
    class_alternatives = []
    if recommendation:
        avail = _build_availability(journey_date, days_before)
        for cls in ["SL", "3A", "2A", "1A", "CC", "EC", "2S"]:
            sc, reason = _confirmation_score_and_reason(
                recommendation["train_number"], cls, journey_date, days_before
            )
            chip = avail.get(cls) or {}
            label = tone = None
            if hasattr(chip, "model_dump"):
                d = chip.model_dump()
                label, tone = d.get("label"), d.get("tone")
            elif hasattr(chip, "label"):
                label = chip.label
                tone = getattr(chip, "tone", None)
            elif isinstance(chip, dict):
                label, tone = chip.get("label"), chip.get("tone")
            class_alternatives.append({"class": cls, "score": sc, "reason": reason, "label": label, "tone": tone})
        class_alternatives.sort(key=lambda x: x["score"], reverse=True)
        class_alternatives = class_alternatives[:3]

    alternative_dates = []
    try:
        base = date.fromisoformat(journey_date)
        for delta in (-1, 0, 1, 2):
            d = base + timedelta(days=delta)
            alternative_dates.append({"date": d.isoformat(), "note": "tatkal opens 1 day before journey"})
    except ValueError:
        alternative_dates = []

    # Show nearby only when there are no direct trains
    shown_nearby = nearby if not direct else []
    out = {
        "source_query": source_q,
        "destination_query": dest_q,
        "trains": direct,
        "direct_trains": direct,
        "nearby_options": shown_nearby,
    ...
    if not candidates:
        ...
        out["tracked_trains_count"] = tracked
    return out
"""
...
_CHAT_BLOCK = r"""
        ...
        if candidates:
            lines = []
            if direct:
                lines.append(f"Direct trains: {len(direct)} found.")
            else:
                lines.append("Direct trains: none in tracked network.")
            # nearby_options is only populated when direct is empty
            if nearby:
                ...
                for n in nearby[:2]:
                    ...
                    hm = f"~{dur // 60}h {dur % 60}m" if dur is not None else "duration n/a"
                    lines.append(
                        f"• {board.get('name') or board.get('code')} → "
                        f"{alight.get('name') or alight.get('code')} via "
                        f"{n.get('train_number')} ({hm})"
                    )
            alts = route_result.get("alternative_dates") or []
            if alts:
                dates = ", ".join(a.get("date") for a in alts if a.get("date"))
                if dates:
                    lines.append(f"Alternative dates: {dates}.")
            classes = route_result.get("class_alternatives") or []
            if classes:
                bits = [f"{c.get('class')} ({c.get('score')}%)" for c in classes]
                lines.append("Class alternatives: " + ", ".join(bits) + "…")
            rec = route_result.get("recommendation")
            if rec:
                number = rec.get("train_number")
                name = number
                try:
                    ...
                    pass
                lines.append(f"Best option: {name} ({number}) — {rec.get('why') or ''}")
            reply = "\n".join(lines).strip()
            return {"reply": reply, "result": route_result}
        # zero candidates → honest empty-state; still try old search() if it has trains
        N = route_result.get("tracked_trains_count") or 0
        empty_reply = (
            f"No direct train in our tracked network yet (we track {N} trains today).\n"
            f"Try: Delhi to Patna, Bhagalpur to Patna, Katihar to Patna, or\n"
            f"Bengaluru to Chennai."
        )
        try:
            query = SearchQuery(
            ...
            result = search(query)
            if result is not None and getattr(result, "best", None) is not None:
                try:
