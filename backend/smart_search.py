...
def _attach_class_preference(rows: list[dict], travel_class: str, journey_date: str) -> None:
    """Attach class preference metadata. Never claim live CNF/RAC/WL or invent %.

    Score is intentionally omitted — heuristic confirmation percentages must not
    appear in the production user flow. Live availability is not connected.
    """
    _ = journey_date
    for r in rows:
        r["requested_class"] = {
            ...
            "score": None,
            "reason": (
                f"Preferred class {travel_class}. Timetable only — "
                "live seat availability is not connected."
            ),
        }
...
def _journey_key(row: dict) -> tuple[str, str, str]:
    """Identity: train_number + board code + alight code."""
    tn = str(row.get("train_number") or "").strip()
    board = (row.get("board") or {}).get("code") or ""
    alight = (row.get("alight") or {}).get("code") or ""
    return (tn, str(board).strip().upper(), str(alight).strip().upper())


def _is_better_candidate(cand: dict, prev: dict) -> bool:
    """Prefer higher rank score, then better data-quality, then shorter duration."""
    sc = _rank_score(cand)
    sp = _rank_score(prev)
    if sc != sp:
        return sc > sp
    bd_c = (cand.get("score_breakdown") or {}).get("data_quality")
    bd_p = (prev.get("score_breakdown") or {}).get("data_quality")
    try:
        dq_c = float(bd_c) if bd_c is not None else 0.0
        dq_p = float(bd_p) if bd_p is not None else 0.0
    except (TypeError, ValueError):
        dq_c = dq_p = 0.0
    if dq_c != dq_p:
        return dq_c > dq_p
    dur_c = cand.get("duration_minutes")
    dur_p = prev.get("duration_minutes")
    if isinstance(dur_c, (int, float)) and isinstance(dur_p, (int, float)) and dur_c != dur_p:
        return float(dur_c) < float(dur_p)
    return _journey_key(cand) < _journey_key(prev)


def _dedupe_best(rows: list[dict]) -> list[dict]:
    """Keep best option per (train_number, board, alight).

    Different boarding or alighting stations for the same train are distinct
    journey options and are NOT collapsed.
    """
    ...
    for r in rows:
        ...
        if not key[0]:
            continue
        prev = best.get(key)
        if prev is None or _is_better_candidate(r, prev):
            best[key] = r
...
def _collect_pairs(
    ...
    for tn, stop_map in trains_idx.items():
        ...
        if best and best_sc and best_dc:
            ...
            best["category"] = cat
            best["station_relation"] = {
                "origin_role": _station_role(best_sc, origin_p_set, origin_sib_set, origin_h_set),
                "dest_role": _station_role(best_dc, dest_p_set, dest_sib_set, dest_h_set),
            }
            best["why"] = _why_v2(
...
def run_smart_search(
    ...
    _apply_v2_scores(direct)
    _apply_v2_scores(alternatives)

    direct = _dedupe_best(direct)
    direct_keys = {_journey_key(r) for r in direct}
    alternatives = [r for r in alternatives if _journey_key(r) not in direct_keys]
    ...
    if pool:
        ...
        recommendation = {
            ...
            "why": _recommend_why_v2(top, source, destination),
            "score": top.get("score"),
            "score_breakdown": top.get("score_breakdown"),
            "station_relation": top.get("station_relation"),
            "label": "Best timetable option",
            "requested_class": top.get("requested_class"),
            "operating_day_verified": False,
            "operating_day_status": "unknown",
            "live": None,
        }
